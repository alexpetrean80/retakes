...
void time_out(int semnal) {
  ...
  close(c); // desi nu am primit nimic de la client in 10 secunde, inchidem
            // civilizat conexiunea cu acesta
  exit(1);
}
...
void tratare() {
  ...
  do {
    ...
    if (cod == 1) // citire cu succes a unui caracter
      alarm(10);  // resetam timerul si asteptam din nou 10 secunde urmatorul
                  // caracter
    if (cod != 1) {
      r = -1;
      break;
    ...
    if (b == ' ') {
      ...
      if (r == INT32_MAX) { // intregul maxim pozitiv pe 4 octetii cu semn sau
                            // 0x7FFFFFFF (a se vedea stdint.h)
        r = -2;
        break;
      }
      r++;
  ...
  while (b != 0); // sirul de caractere de la client se considera terminat la
                  // intalnirea caracterului 0 (NULL, '\0')

  // Paragraful 2 - terminam citirea sirului de la client la primirea
  // caracterului NULL
  ...
  if (r >= 0)
    ...
    printf(
        "Am inchis cu succes conexiunea cu un client. I-am trimis %d spatii.\n",
        r);
  ...
  // Terminam procesul fiu - foarte important!!! altfel numarul de procese
  // creste exponential.
...
int main() {
  ...
  while (1) { // deserveste oricati clienti
    ...
    if (fork() == 0) { // server concurent, conexiunea va fi tratata de catre un
                       // proces fiu separat
